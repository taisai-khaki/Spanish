...
t('B1 is the main stage (708 items: 545w/82s/45g/4 dialogues, 36 lines)', run('engine.stats("B1").total') === 708
  && run('DATA.B1.words.length') === 545 && run('DATA.B1.sentences.length') === 82
  && run('DATA.B1.grammar.length') === 45 && run('DATA.B1.dialogues.length') === 4);
...
t('game pool total = 1053 items across A1–B2 (exam not counted)',
  run('engine.stats("A1").total + engine.stats("A2").total + engine.stats("B1").total + engine.stats("B2").total') === 1053);
